// web/src/pages/exams/annual-results/index.tsx
...
import type { AnnualResult } from "@/types/exam.types";
...
const PROMO_COLORS: Record<string, string> = {
  ...
  repeated:   "bg-red-50 text-red-700",
...
export default function AnnualResultsPage() {
  ...
  const { data, isLoading } = useAnnualResults(academicYear, undefined, page);
  const computeMutation = useComputeAnnualResults();
  const publishMutation = usePublishAnnualResults();
  ...
  const avg = results.length
    ? (results.reduce((s, r) => s + r.annualAverage, 0) / results.length).toFixed(1)
    : "—";
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900">{t("annualResults.title")}</h1>
            <p className="text-sm text-gray-500">{t("annualResults.description")}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-5">
        ...
        <SummaryCard icon={TrendingUp} label={t("annualResults.schoolAverage")} value={avg} />
        <SummaryCard icon={Award} label={t("annualResults.passed")} value={String(passed)} />
        <SummaryCard icon={Award} label={t("annualResults.passRate")} value={`${passRate}%`} />
        <SummaryCard icon={GraduationCap} label={t("annualResults.promoted")} value={String(promoted)} />
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50 text-left text-xs font-medium uppercase tracking-wider text-gray-500">
              ...
              <th className="px-4 py-3">{t("annualResults.studentName")}</th>
              <th className="px-4 py-3">{t("annualResults.admissionNo")}</th>
              <th className="px-4 py-3">{t("annualResults.className")}</th>
              <th className="px-4 py-3 text-center">{t("annualResults.term1Avg")}</th>
              <th className="px-4 py-3 text-center">{t("annualResults.term2Avg")}</th>
              <th className="px-4 py-3 text-center">{t("annualResults.term3Avg")}</th>
              <th className="px-4 py-3 text-center">{t("annualResults.annualAverage")}</th>
              <th className="px-4 py-3 text-center">{t("annualResults.grade")}</th>
              <th className="px-4 py-3 text-center">{t("annualResults.position")}</th>
              <th className="px-4 py-3 text-center">{t("annualResults.promotion")}</th>
            </tr>
          </thead>
            ...
            {isLoading ? (
              ...
              <tr>
                <td colSpan={11} className="px-4 py-12 text-center text-gray-400">
                  {t("annualResults.noResults")}
                </td>
              </tr>
            ) : (
              results.map((r, idx) => {
                const t1 = r.termAverages?.find((t) => t.term === 1);
                ...
                return (
                  <tr key={r._id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 text-center text-gray-700">
                      {t1?.average?.toFixed(1) ?? "—"}
                    </td>
                    <td className="px-4 py-3 text-center text-gray-700">
                      {t2?.average?.toFixed(1) ?? "—"}
                    </td>
                    <td className="px-4 py-3 text-center text-gray-700">
                      {t3?.average?.toFixed(1) ?? "—"}
                    </td>
                    <td className="px-4 py-3 text-center font-semibold text-gray-900">
                      {r.annualAverage?.toFixed(1) ?? "—"}
                    </td>
                    ...
                    <td className="px-4 py-3 text-center">
                      <span
                        className={cn(
                          ...
                          PROMO_COLORS[r.promotionStatus] ?? "bg-gray-50 text-gray-500"
                        )}
                      >
                        {t(`annualResults.status_${r.promotionStatus}`)}
                      </span>
                    </td>
                  </tr>
                );
