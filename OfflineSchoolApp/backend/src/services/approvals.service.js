...
// The threshold rule, shared with the desktop application.
//
// It records expenses with no connection, and whether one needs approval decides
// what it may honestly do offline: below the threshold it writes the row and
// queues the request, and at or above it cannot, because approval needs a second
// person who is not on that machine.
//
// The boundary is AT or above, and that single case is exactly what two copies
// of this rule would eventually disagree about — an expense of precisely the
// threshold slipping into the accounts unsigned on one platform and not the
// other. See OfflineSchoolApp/shared/approvalThresholds.js.
const {
  whole,
  DEFAULT_THRESHOLDS,
  resolveThresholds,
