      ...
      <View style={styles.assignmentInfo}>
        ...
        <View style={styles.assignmentMeta}>
          <View style={styles.metaChip}>
            ...
            <Text style={styles.metaChipText}>{assignment.subject?.name || "N/A"}</Text>
          </View>
          <View style={styles.metaChip}>
            <Ionicons name="school-outline" size={12} color="#7C3AED" />
            <Text style={styles.metaChipText}>{assignment.class?.name || "N/A"}</Text>
          </View>
        </View>
      </View>
    </View>
...
const GroupHeader = React.memo(
  ({ title, subtitle, avatarContent, badgeCount, badgeColor, isExpanded, onToggle }) => (
    <TouchableOpacity style={styles.groupHeader} onPress={onToggle} activeOpacity={0.7}>
      <View style={styles.groupHeaderLeft}>
        <View style={[styles.groupAvatar, { backgroundColor: badgeColor + "20" }]}>
          {avatarContent}
        </View>
        <View style={{ flex: 1 }}>
      ...
      <View style={styles.groupHeaderRight}>
        <View style={[styles.countBadge, { backgroundColor: badgeColor + "20" }]}>
          <Text style={[styles.countBadgeText, { color: badgeColor }]}>{badgeCount}</Text>
        </View>
...
const UnassignedCard = React.memo(({ teacher, onAssign }) => (
  <View style={styles.unassignedCard}>
    <View style={styles.unassignedLeft}>
      ...
      <View style={{ flex: 1 }}>
        <Text style={styles.unassignedName} numberOfLines={1}>{teacher.name}</Text>
        <Text style={styles.unassignedEmail} numberOfLines={1}>{teacher.email || "No email"}</Text>
      </View>
    </View>
    <TouchableOpacity
...
const EmptyState = React.memo(({ icon, title, subtitle, color = "#374151" }) => (
...
export default function AssignmentsIndex() {
  ...
  useFocusEffect(
    useCallback(() => {
      let active = true;
      const run = async () => { if (active) loadData(); };
      run();
      return () => { active = false; };
    }, [loadData])
  );
  ...
  const handleRemoveAssignment = useCallback((assignment) => {
    Alert.alert(
      "Remove Assignment",
      `Remove ${assignment.teacher?.name || "this teacher"} from ${assignment.subject?.name || "this subject"} in ${assignment.class?.name || "this class"}?`,
      [
  ...
  const groupedByTeacher = useMemo(() =>
    assignments.reduce((acc, assignment) => {
      const teacherId = assignment.teacher?._id || assignment.teacher?.id || "unknown";
      if (!acc[teacherId]) {
        acc[teacherId] = {
  ...
  const groupedByClass = useMemo(() =>
    assignments.reduce((acc, assignment) => {
      const classId = assignment.class?._id || assignment.class?.id || "unknown";
      if (!acc[classId]) {
        acc[classId] = {
  ...
  const renderAllTab = useCallback(() =>
    filteredAssignments.length === 0 ? (
      <EmptyState
        ...
        subtitle={searchQuery ? "Try a different search term" : "Start by assigning teachers to subjects"}
      />
    ) : (
      <>
  ...
  const renderByTeacherTab = useCallback(() => {
    ...
    if (groups.length === 0) {
      return (
        <EmptyState icon="people-outline" title="No Teacher Assignments"
          subtitle="Assign teachers to subjects to see them here" />
      );
    }

    const rows = groups.map(([teacherId, group]) => {
                ...
                <View key={assignment._id} style={styles.groupItem}>
                  <View style={styles.groupItemLeft}>
                    ...
                    <View style={styles.classPill}>
                      <Text style={styles.classPillText}>{assignment.class?.name || "N/A"}</Text>
                    </View>
                  </View>
                  <TouchableOpacity
    ...
    if (searchQuery && rows.length === 0) {
      return <EmptyState icon="search-outline" title="No Matches" subtitle="Try a different search term" />;
    }
    return <>{rows}</>;
  }, [groupedByTeacher, expandedGroups, searchQuery, matchesSearch, toggleGroup, handleRemoveAssignment, deletingId]);

  const renderByClassTab = useCallback(() => {
    const groups = Object.entries(groupedByClass);
    if (groups.length === 0) {
      return (
        <EmptyState icon="school-outline" title="No Class Assignments"
          subtitle="Assign teachers to class subjects first" />
      );
    }

    const rows = groups.map(([classId, group]) => {
      ...
      return (
        <View key={classId} style={styles.groupContainer}>
          <GroupHeader
            ...
            avatarContent={<Ionicons name="school" size={18} color="#7C3AED" />}
            badgeCount={group.assignments.length}
            badgeColor="#7C3AED"
            isExpanded={isExpanded}
            onToggle={() => toggleGroup(key)}
              ...
              {displayList.map((assignment) => (
                <View key={assignment._id} style={styles.groupItem}>
                  <View style={styles.groupItemLeft}>
                    ...
                    <View style={[styles.classPill, { backgroundColor: "#ECFDF5" }]}>
    ...
    return <>{rows}</>;
  }, [groupedByClass, expandedGroups, searchQuery, matchesSearch, toggleGroup, handleRemoveAssignment, deletingId]);
      ...
      <View>
        <View style={styles.unassignedBanner}>
          ...
          <Text style={styles.unassignedBannerText}>
            {unassignedTeachers.length} teacher{unassignedTeachers.length !== 1 ? "s" : ""} without any subject assignment
          </Text>
        </View>
        {unassignedTeachers.map((teacher) => (
          <UnassignedCard key={teacher._id} teacher={teacher} onAssign={handleAssignTeacher} />
        ))}
      </View>
    )
  ...
  }, [activeTab, renderAllTab, renderByTeacherTab, renderByClassTab, renderUnassignedTab]);
            ...
            >
              ...
              {tab.id === "unassigned" && unassignedTeachers.length > 0 && (
                <View style={styles.tabBadge}>
                  <Text style={styles.tabBadgeText}>{unassignedTeachers.length}</Text>
                </View>
              )}
            </TouchableOpacity>
          ))}
...
const styles = StyleSheet.create({
  ...
  statsContainer: {
    flexDirection: "row", paddingHorizontal: 20, gap: 8, marginTop: 20,
  },
  statCard: {
