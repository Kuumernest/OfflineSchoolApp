...
// IMPORT ADDED: Using the shared helper instead of local implementation
...
// UPDATED: Now uses the shared dbHelper just like the teacher file
...
const loadStudents = async (classId, schoolId) => {
  ...
  console.log("[loadStudents] columns:", colNames.join(", "));
  console.log("[loadStudents] resolved →",
    `schoolCol=${schoolCol} classCol=${classCol}`,
    `deletedCol=${deletedCol} statusCol=${statusCol}`
  );
  ...
  console.log("[loadStudents] SQL:", sql.replace(/\s+/g, " ").trim());
  const rows = await dbQuery(sql, params);

  // JS-level dedup by normalised name (safety net)
  const seen   = new Set();
