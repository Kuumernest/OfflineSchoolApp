...
const QUICK_ACTIONS = [
  { id: "add-class",       title: "Add Class",      icon: "add-circle-outline", color: "#4F46E5", route: "/admin/classes/add"           },
  { id: "add-subject",     title: "Add Subject",    icon: "add-circle-outline", color: "#059669", route: "/admin/subjects/add"          },
  { id: "add-teacher",     title: "Add Teacher",    icon: "add-circle-outline", color: "#7C3AED", route: "/admin/teachers/add"          },
  { id: "assign-teacher",  title: "Assign Teacher", icon: "git-branch-outline", color: "#DB2777", route: "/admin/assignments"           },
  { id: "review-apps",     title: "Review Apps",    icon: "clipboard-outline",  color: "#D97706", route: "/admin/students/applications" },
  { id: "build-timetable", title: "Timetable",      icon: "time-outline",       color: "#DC2626", route: "/admin/timetable"             },
];

const ALL_MODULES = [
  { id: "classes",             title: "Classes",       icon: "school-outline",        color: "#4F46E5", route: "/admin/classes",               description: "Create & manage classes"  },
  { id: "subjects",            title: "Subjects",      icon: "book-outline",          color: "#059669", route: "/admin/subjects",              description: "Create & link subjects"   },
  { id: "teachers",            title: "Teachers",      icon: "people-outline",        color: "#7C3AED", route: "/admin/teachers",              description: "Manage teachers"          },
  { id: "applications",        title: "Applications",  icon: "person-add-outline",    color: "#D97706", route: "/admin/students/applications", description: "Review applications"      },
  { id: "students",            title: "Students",      icon: "people-circle-outline", color: "#059669", route: "/admin/students/approved",     description: "Approved students"        },
  { id: "teacher-assignments", title: "Assignments",   icon: "git-branch-outline",    color: "#DB2777", route: "/admin/assignments",           description: "Teacher allocation"       },
  { id: "timetable",           title: "Timetable",     icon: "time-outline",          color: "#DC2626", route: "/admin/timetable",             description: "Schedule builder"         },
  { id: "attendance",          title: "Attendance",    icon: "calendar-outline",      color: "#059669", route: "/admin/attendance",            description: "Tracking & reports"       },
  { id: "exams",               title: "Exams",         icon: "trophy-outline",        color: "#7C3AED", route: "/admin/exams",                 description: "Exams & results"          },
  { id: "announcements",       title: "Announcements", icon: "megaphone-outline",     color: "#DB2777", route: "/admin/announcements",         description: "Broadcast system"         },
  { id: "settings",            title: "Settings",      icon: "settings-outline",      color: "#6B7280", route: "/admin/settings",             description: "System config"            },
];
...
const AlertRow = React.memo(({ alert, onPress }) => {
  ...
  return (
    ...
    >
      ...
      <Text style={[styles.alertText, { color: st.text }]}>{alert.message}</Text>
...
export default function AdminDashboard() {
  ...
  const loadStats = useCallback(async (isRefresh = false, forceSync = false) => {
    try {
      setError(null);
  ...
  useEffect(() => { loadStats(false, false); }, []);
  ...
  const navigate = useCallback((path, title) => {
    if (!path) {
  ...
  const alerts = useMemo(() => {
    ...
    if (stats.stalePendingApps > 0) {
      list.push({
        id: "stale", type: "danger", icon: "alert-circle-outline",
        message: `${stats.stalePendingApps} application${stats.stalePendingApps > 1 ? "s" : ""} pending over 3 days`,
        route: "/admin/students/applications",
      });
    }
    if (stats.unassignedTeachers > 0) {
      list.push({
        id: "unassigned", type: "warning", icon: "people-outline",
        message: `${stats.unassignedTeachers} teacher${stats.unassignedTeachers > 1 ? "s" : ""} not yet assigned`,
        route: "/admin/assignments",
      });
    }
    if (stats.classesWithoutSubjects > 0) {
      list.push({
        id: "missing-subjects", type: "warning", icon: "book-outline",
        message: `${stats.classesWithoutSubjects} class${stats.classesWithoutSubjects > 1 ? "es" : ""} missing subjects`,
        route: "/admin/subjects",
      });
    }
    if (stats.timetableConflicts > 0) {
      list.push({
        id: "conflicts", type: "danger", icon: "warning-outline",
        message: `${stats.timetableConflicts} timetable conflict${stats.timetableConflicts > 1 ? "s" : ""} detected`,
    ...
    if (stats.incompleteTimetableSlots > 0 && stats.totalClasses > 0) {
      ...
      if (pct > 50) {
        list.push({
          id: "timetable-incomplete", type: "info", icon: "calendar-outline",
          message: `${stats.incompleteTimetableSlots} class${stats.incompleteTimetableSlots > 1 ? "es" : ""} without timetable (${pct}%)`,
          route: "/admin/timetable",
        });
      }
    }
    if (
      ...
      list.push({
        id: "no-assignments", type: "warning", icon: "git-branch-outline",
  ...
  const statValue = useCallback((key) => stats?.[key] ?? 0, [stats]);

  const visibleModules = useMemo(
    () => showAllModules ? ALL_MODULES : ALL_MODULES.slice(0, MODULES_PREVIEW_COUNT),
    [showAllModules]
  );
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={styles.section}>
          ...
          {STAT_ROWS.map((row) => (
            <View key={row.map((s) => s.key).join("-")} style={styles.statsRow}>
              {row.map((s) => (
        ...
        {alerts.length > 0 && (
          <View style={styles.section}>
            <Text style={styles.sectionTitle}>
              Alerts <Text style={styles.alertBadge}>({alerts.length})</Text>
            </Text>
            {alerts.map((a) => (
              <AlertRow
                key={a.id}
...
const styles = StyleSheet.create({
  ...
  centered:      { flex: 1, justifyContent: "center", alignItems: "center", backgroundColor: "#F3F4F6", gap: 10 },
  ...
  actionTitle: { fontSize: 11, textAlign: "center", color: "#374151", fontWeight: "600" },
