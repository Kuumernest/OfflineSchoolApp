// app/admin/attendance/report/students.js
  ...
  for (const r of records) {
    ...
    if (!byStudent[id]) {
      ...
      const name =
        ...
        r.student?.name  ||
        t("attAdmin.unknownStudent");

      byStudent[id] = {
        studentId:    id,
        studentName:  name,
        studentEmail: info.email       || r.studentEmail || r.email || "",
        classId:      info.classId     || r.classId      || r.class_id || null,
        className:    info.className   || r.className    || null,
        admissionNo:  info.admissionNo || r.admissionNo  || null,
        records:      [],
    ...
    if (key in byStudent[id].summary) {
      byStudent[id].summary[key]++;
    }
...
const StudentReportCard = React.memo(({ student, isExpanded, onPress }) => {
  const { t } = useTranslation();
  const { summary, rate } = student;
  const color = rateColor(rate);

  const subLine = student.className
...
const StudentDetailPanel = React.memo(({ student }) => {
  const { t } = useTranslation();
  ...
  return (
    <View style={detailS.container}>
      <Text style={detailS.title}>
        Attendance Records ({student.records.length})
      </Text>
      {sorted.map((rec, i) => {
...
export default function StudentAttendanceReportScreen() {
  ...
  const loadReport = useCallback(async (isRefresh = false) => {
    try {
      ...
      console.log(`📋 Raw student records: ${rawRecords.length}`);
      ...
      let studentMap = {};
      try {
        if (typeof AttendanceService.buildStudentMap === "function") {
          studentMap = await AttendanceService.buildStudentMap(schoolId);
          console.log(
            `🗺️ Student map: ${Object.keys(studentMap).length} entries`
          );
        }
      } catch (mapErr) {
      ...
      console.log(
        `📊 Student report: ${rawRecords.length} records → ${summaries.length} students`
      );

    } catch (err) {
      console.error("Student report load failed:", err?.message || err);
  ...
  if (loading) {
    return (
      <View style={styles.centered}>
        ...
        <Text style={styles.loadingText}>{t("attAdmin.loadingStudentReport")}</Text>
      </View>
    );
  }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>{t("attAdmin.studentReportTitle")}</Text>
          <Text style={styles.headerSub}>
            {t(selectedPreset.labelKey)} · {t("academic.studentCount", { count: filtered.length })}
          </Text>
        </View>
      ...
      <View style={styles.searchRow}>
        <View style={styles.searchBox}>
          ...
          <TextInput
            ...
            placeholder={t("attAdmin.searchStudentReport")}
      ...
      <View style={styles.sortRow}>
        {[
          { key: "name",   label: "A – Z"       },
          { key: "class",  labelKey: "attAdmin.sortByClass"    },
        ...
        renderItem={({ item }) => (
          <View>
            ...
            {expandedId === item.studentId && (
              <StudentDetailPanel student={item} />
            )}
          </View>
        )}
        ListEmptyComponent={
